...
export const useTest = () => {
  ...
  const GetTestById = async(id: string) => {
    const response = await $fetch<{data: Test }>(`${BASE_URL}/api/guru/tes/show/${id}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'Accept': 'application/json',
  ...
  const EditTest = async ( id: string, formData: Test) => {
    const response = await $fetch<Test>(`${BASE_URL}/api/guru/tes/update/${id}`, {
      method: 'put',
      headers: {
        'Content-Type': 'application/json',
        'Accept': 'application/json',
  ...
  return {GetTest, CreateTest, EditTest, GetTestById}
